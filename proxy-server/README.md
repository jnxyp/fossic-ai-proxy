...
每次请求自动写入 `data/usage.db`（SQLite），记录字段：

| 字段 | 说明 |
|------|------|
| `ts` | 请求时间（UTC） |
| `tenant` | 租户名称 |
| `key_hint` | 所用 Key 后 8 位（`...xxxxxxxx`） |
| `model` | 模型名 |
| `stream` | 是否流式（0/1） |
| `input_chars` | 请求字符数 |
| `output_chars` | 响应字符数 |
...
```sql
-- 按租户统计总请求数和字符数
SELECT tenant, COUNT(*) AS reqs, SUM(input_chars) AS in_chars, SUM(output_chars) AS out_chars
FROM requests GROUP BY tenant;

-- 最近 7 天每日请求量
SELECT DATE(ts) AS day, COUNT(*) AS reqs
FROM requests WHERE ts >= DATE('now', '-7 days')
GROUP BY day ORDER BY day;
```

`data/` 目录通过 volume 挂载持久化，`usage.db` 不入库。
